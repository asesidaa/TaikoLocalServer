...
public static partial class PlayResultMappers
{
    ...
    private static List<uint> MapToneFlags(byte[]? values)
        => BitsetCodec.Decode(values, Ac15EraProfiles.Murasaki.Limits.ToneFlagBytes);

    private static List<uint> MapCostumeFlags(byte[]? values)
        => BitsetCodec.Decode(values, Ac15EraProfiles.Murasaki.Limits.CostumeFlagBytes);

    private static List<uint> MapTitleFlags(byte[]? values)
        => BitsetCodec.Decode(values, Ac15EraProfiles.Murasaki.Limits.TitleFlagBytes);
