...
public static partial class PlayResultMappers
{
    ...
    [MapProperty(nameof(PlayResultRequest.StageData.HitCnt), nameof(Ac15StageResult.HitCount))]
    ...
    private static List<uint> MapToneFlags(byte[]? values)
        => Ac15MapperNormalization.DecodeBitset(values, Ac15EraProfiles.Murasaki.Limits.ToneFlagBytes);

    private static List<uint> MapCostumeFlags(byte[]? values)
        => Ac15MapperNormalization.DecodeBitset(values, Ac15EraProfiles.Murasaki.Limits.CostumeFlagBytes);

    private static List<uint> MapTitleFlags(byte[]? values)
        => Ac15MapperNormalization.DecodeBitset(values, Ac15EraProfiles.Murasaki.Limits.TitleFlagBytes);
