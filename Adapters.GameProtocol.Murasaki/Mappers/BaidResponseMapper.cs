...
public static partial class BaidResponseMapper
{
    ...
    private static byte[] MapCostumeFlag(byte[]? value)
        => Ac15MapperNormalization.FixedOrZero(value, Ac15EraProfiles.Murasaki.Limits.CostumeFlagBytes);

    private static byte[] MapDanFlag(byte[]? value)
        => Ac15MapperNormalization.FixedOrZero(value, Ac15EraProfiles.Murasaki.Limits.DanFlagBytes);
}
