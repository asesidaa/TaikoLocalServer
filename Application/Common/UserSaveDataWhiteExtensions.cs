...
public static class UserSaveDataWhiteExtensions
{
    public static async ValueTask<UserSaveDataWhite> GetOrCreateWhiteSaveDataAsync(
        this ITaikoDbContext context,
        uint baid,
        CancellationToken cancellationToken = default)
    {
        var saveData = await context.UserSaveDataWhite.FindAsync([baid], cancellationToken);
        if (saveData is not null)
        {
            return saveData;
        }

        saveData = CreateDefaultWhiteSaveData(baid);
        context.UserSaveDataWhite.Add(saveData);
        return saveData;
    }

    public static UserSaveDataWhite CreateDefaultWhiteSaveData(uint baid)
    {
        var limits = Ac15EraProfiles.White.Limits;
        return new UserSaveDataWhite
        {
            Baid = baid,
            Title = string.Empty,
            TitleplateId = 0,
