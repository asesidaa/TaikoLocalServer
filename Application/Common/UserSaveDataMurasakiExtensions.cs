...
public static class UserSaveDataMurasakiExtensions
{
    public static async ValueTask<UserSaveDataMurasaki> GetOrCreateMurasakiSaveDataAsync(
        this ITaikoDbContext context,
        uint baid,
        CancellationToken cancellationToken = default)
    {
        var saveData = await context.UserSaveDataMurasaki.FindAsync([baid], cancellationToken);
        if (saveData is not null)
        {
            return saveData;
        }

        saveData = CreateDefaultMurasakiSaveData(baid);
        context.UserSaveDataMurasaki.Add(saveData);
        return saveData;
    }

    public static UserSaveDataMurasaki CreateDefaultMurasakiSaveData(uint baid)
    {
        var limits = Ac15EraProfiles.Murasaki.Limits;
        return new UserSaveDataMurasaki
        {
            Baid = baid,
            Title = string.Empty,
            TitleplateId = 0,
