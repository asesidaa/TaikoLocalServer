...
{
    private partial async ValueTask<Ac15BaidResponse> HandleMurasaki(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var saveData = await context.UserSaveDataMurasaki.FindAsync([card.Baid], cancellationToken);
        if (saveData is null)
        {
            return new Ac15BaidResponse
            {
        ...
        var userData = await context.UserData.FindAsync([card.Baid], cancellationToken)
            ?? throw new InvalidOperationException($"User not found for Murasaki card baid {card.Baid}.");
