...
{
    private partial async ValueTask<Ac15BaidResponse> HandleWhite(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var saveData = await context.UserSaveDataWhite.FindAsync([card.Baid], cancellationToken);
        if (saveData is null)
        {
            return new Ac15BaidResponse
            {
        ...
        var userData = await context.UserData.FindAsync([card.Baid], cancellationToken)
            ?? throw new InvalidOperationException($"User not found for White card baid {card.Baid}.");

        var limits = Ac15EraProfiles.White.Limits;
        var mydonProfile = new Ac15BaidProfile
        {
            Title = saveData.Title,
            TitlePlateId = ResolveWhiteTitlePlateId(saveData),
    ...
    private uint ResolveWhiteTitlePlateId(UserSaveDataWhite saveData)
    {
        return gameDataService.White().GetTitleDictionary().TryGetValue(saveData.TitleplateId, out var title)
               && WhiteTitleTextMatches(title, saveData.Title)
            ? title.TitleRarity
            : saveData.TitleplateId;
    }

    private static bool WhiteTitleTextMatches(Title title, string selectedTitle)
    {
