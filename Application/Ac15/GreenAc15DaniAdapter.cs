...
public sealed class GreenAc15DaniAdapter(ITaikoDbContext context, IEnumerable<uint> knownChallengeLevels)
    : IAc15DaniPersistence
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var stage in score.Stages)
        {
            ...
            if (stageRow is null)
            {
                stageRow = new DanStageScoreDatumGreen
                {
                    Baid = score.Baid,
                    DanId = score.DanId,
                    IsExtra = score.IsExtra,
    ...
    private static Ac15DaniScore Map(DanScoreDatumGreen row)
        => new(
            row.Baid,
            row.DanId,
            row.IsExtra,
    ...
    private static Ac15DaniStageScore MapStage(DanStageScoreDatumGreen stage)
        => new(
            stage.StageIndex,
            stage.SongNumber,
            stage.PlayScore,
