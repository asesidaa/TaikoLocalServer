...
public sealed class BlueAc15DaniAdapter(ITaikoDbContext context, IEnumerable<uint> knownChallengeLevels)
    : IAc15DaniPersistence
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var stage in score.Stages)
        {
            ...
            if (stageRow is null)
            {
                stageRow = new DanStageScoreDatumBlue
                {
                    Baid = score.Baid,
                    DanId = score.DanId,
                    IsExtra = score.IsExtra,
    ...
    private static Ac15DaniScore Map(DanScoreDatumBlue row)
        => new(
            row.Baid,
            row.DanId,
            row.IsExtra,
    ...
    private static Ac15DaniStageScore MapStage(DanStageScoreDatumBlue stage)
        => new(
            stage.StageIndex,
            stage.SongNumber,
            stage.PlayScore,
