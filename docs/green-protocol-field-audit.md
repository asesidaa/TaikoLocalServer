...
| `gettelop.php` | telop fields | `GetTelopController.cs`; `GetTelopQuery.Green.cs`; `GreenTelopLoader.cs`; `GetTelopMappers.cs` | Source-inferred telop fetch; IDA shows no `OnGettelopResponse` callback — proto-parsed string fields are stored verbatim, no `stoi`/`sscanf`/`strptime` consumer found. | Datetime strings safe at fixed-width `yyyyMMddHHmmss`; unknown telop_id must omit optional fields. | Reads `wwwroot/data/green/telop_data.json`; advertises rows in `initialdatacheck`; mapper omits optionals when absent; unknown id returns `result=1` only. | Low. | `GreenTelopTests` (loader, initialdata advertise, mapper omission, unknown-id safety). |
