...
internal sealed class WhiteHandlerFixture : IAsyncDisposable
{
    private readonly SqliteConnection connection;

    private WhiteHandlerFixture(SqliteConnection connection, TaikoDbContext context, IGameDataCatalog catalog)
    {
    ...
    public static async Task<WhiteHandlerFixture> CreateAsync(IWhiteCatalog? whiteCatalog = null)
    {
        ...
        var catalog = new FileGameDataCatalog([whiteCatalog ?? new TestWhiteCatalog()]);
        return new WhiteHandlerFixture(connection, context, catalog);
    }
    ...
    internal sealed class TestWhiteCatalog : IWhiteCatalog
    {
        ...
        public uint SongHashVersion => 707;
        ...
        public IReadOnlyDictionary<uint, Ac15MusicInfoEntry> WhiteMusicInfos
        ...
        [
            ...
            new() { CostumeId = 36, CostumeType = "kigurumi" }
        ];

        public IReadOnlyDictionary<uint, Title> TitleDictionary { get; init; } =
            new Dictionary<uint, Title>
            {
                [10] = new() { TitleId = 10, TitleName = "White Title", TitleRarity = 0 }
            };
