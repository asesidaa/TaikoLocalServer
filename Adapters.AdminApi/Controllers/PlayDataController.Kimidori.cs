...
{
    private async Task<SongBestResponse> BuildKimidoriSongBestResponse(uint baid)
    {
        var bestRows = await context.SongBestDataKimidori
            .Where(d => d.Baid == baid)
            .AsNoTracking()
            .ToListAsync(HttpContext.RequestAborted);
        var playRows = await context.SongPlayDataKimidori
            .Where(d => d.Baid == baid)
            .AsNoTracking()
            .ToListAsync(HttpContext.RequestAborted);
        ...
        var songBestRecords = bestRows
            ...
            .Select(group => BuildKimidoriSongBestData(group, playRows, favoriteSet))
            .ToList();
    ...
    private static SongBestData BuildKimidoriSongBestData(
        IEnumerable<SongBestDatumKimidori> bestRows,
        IReadOnlyCollection<SongPlayDatumKimidori> playRows,
        IReadOnlySet<uint> favoriteSet)
    {
        ...
        var result = new SongBestData
        {
            ...
            RecentPlayData = matchingPlays.Select(MapKimidoriPlayToDto).ToList()
        };
    ...
    private static SongPlayDatumDto MapKimidoriPlayToDto(SongPlayDatumKimidori play)
    {
