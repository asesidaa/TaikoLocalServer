...
{
    private async Task<DanBestDataResponse> BuildKimidoriDanBestData(uint baid)
    {
        var rows = await context.DanScoreDataKimidori
            .Where(d => d.Baid == baid)
        ...
        return new DanBestDataResponse
        {
            DanBestDataList = rows.Select(row => ToAc15DanBestData(
                ...
                MapKimidoriClearGrade)).ToList()
        };
    }

    private static DanClearState MapKimidoriClearGrade(Ac15DanClearGrade grade)
    {
        return grade switch
        {
