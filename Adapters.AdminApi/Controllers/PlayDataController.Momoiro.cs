...
{
    private async Task<SongBestResponse> BuildMomoiroSongBestResponse(uint baid)
    {
        var bestRows = await context.SongBestDataMomoiro
            .Where(d => d.Baid == baid)
            .AsNoTracking()
            .ToListAsync(HttpContext.RequestAborted);
        var playRows = await context.SongPlayDataMomoiro
            .Where(d => d.Baid == baid)
            .AsNoTracking()
            .ToListAsync(HttpContext.RequestAborted);
        var favoriteSet = await context.MomoiroFavoriteSongs
            .Where(d => d.Baid == baid)
            .Select(d => d.SongNo)
        ...
        var songBestRecords = bestRows
            ...
            .Select(group => BuildMomoiroSongBestData(group, playRows, favoriteSet))
            .ToList();
    ...
    private static SongBestData BuildMomoiroSongBestData(
        IEnumerable<SongBestDatumMomoiro> bestRows,
        IReadOnlyCollection<SongPlayDatumMomoiro> playRows,
        IReadOnlySet<uint> favoriteSet)
    {
        ...
        var result = new SongBestData
        {
            ...
            RecentPlayData = matchingPlays.Select(MapMomoiroPlayToDto).ToList()
        };
    ...
    private static SongPlayDatumDto MapMomoiroPlayToDto(SongPlayDatumMomoiro play)
    {
