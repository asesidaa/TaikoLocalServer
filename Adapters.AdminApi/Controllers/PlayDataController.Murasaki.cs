...
{
    private async Task<SongBestResponse> BuildMurasakiSongBestResponse(uint baid)
    {
        var bestRows = await context.SongBestDataMurasaki
            .Where(d => d.Baid == baid)
            .AsNoTracking()
            .ToListAsync(HttpContext.RequestAborted);
        var playRows = await context.SongPlayDataMurasaki
            .Where(d => d.Baid == baid)
            .AsNoTracking()
            .ToListAsync(HttpContext.RequestAborted);
        var favoriteSet = await context.MurasakiFavoriteSongs
            .Where(d => d.Baid == baid)
            .Select(d => d.SongNo)
        ...
        var songBestRecords = bestRows
            ...
            .Select(group => BuildMurasakiSongBestData(group, playRows, favoriteSet))
            .ToList();
    ...
    private static SongBestData BuildMurasakiSongBestData(
        IEnumerable<SongBestDatumMurasaki> bestRows,
        IReadOnlyCollection<SongPlayDatumMurasaki> playRows,
        IReadOnlySet<uint> favoriteSet)
    {
        ...
        var result = new SongBestData
        {
            ...
            RecentPlayData = matchingPlays.Select(MapMurasakiPlayToDto).ToList()
        };
    ...
    private static SongPlayDatumDto MapMurasakiPlayToDto(SongPlayDatumMurasaki play)
    {
