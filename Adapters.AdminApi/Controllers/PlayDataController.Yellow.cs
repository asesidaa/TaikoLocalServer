...
{
    private async Task<SongBestResponse> BuildYellowSongBestResponse(uint baid)
    {
        var bestRows = await context.SongBestDataYellow
            .Where(d => d.Baid == baid)
            .AsNoTracking()
            .ToListAsync(HttpContext.RequestAborted);
        var playRows = await context.SongPlayDataYellow
            .Where(d => d.Baid == baid)
            .AsNoTracking()
            .ToListAsync(HttpContext.RequestAborted);
        ...
        var songBestRecords = bestRows
            ...
            .Select(group => BuildYellowSongBestData(group, playRows, favoriteSet))
            .ToList();
    ...
    private static SongBestData BuildYellowSongBestData(
        IEnumerable<SongBestDatumYellow> bestRows,
        IReadOnlyCollection<SongPlayDatumYellow> playRows,
        IReadOnlySet<uint> favoriteSet)
    {
        ...
        var result = new SongBestData
        {
            ...
            RecentPlayData = matchingPlays.Select(MapYellowPlayToDto).ToList()
        };
    ...
    private static SongPlayDatumDto MapYellowPlayToDto(SongPlayDatumYellow play)
    {
