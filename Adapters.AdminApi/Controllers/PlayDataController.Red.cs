...
{
    private async Task<SongBestResponse> BuildRedSongBestResponse(uint baid)
    {
        var bestRows = await context.SongBestDataRed
            .Where(d => d.Baid == baid)
            .AsNoTracking()
            .ToListAsync(HttpContext.RequestAborted);
        var playRows = await context.SongPlayDataRed
            .Where(d => d.Baid == baid)
            .AsNoTracking()
            .ToListAsync(HttpContext.RequestAborted);
        var favoriteSet = await context.RedFavoriteSongs
            .Where(d => d.Baid == baid)
            .Select(d => d.SongNo)
        ...
        var songBestRecords = bestRows
            ...
            .Select(group => BuildRedSongBestData(group, playRows, favoriteSet))
            .ToList();
    ...
    private static SongBestData BuildRedSongBestData(
        IEnumerable<SongBestDatumRed> bestRows,
        IReadOnlyCollection<SongPlayDatumRed> playRows,
        IReadOnlySet<uint> favoriteSet)
    {
        ...
        var result = new SongBestData
        {
            ...
            RecentPlayData = matchingPlays.Select(MapRedPlayToDto).ToList()
        };
    ...
    private static SongPlayDatumDto MapRedPlayToDto(SongPlayDatumRed play)
    {
