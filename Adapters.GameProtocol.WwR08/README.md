...
Inbound adapter for the **39.06 WW** game client (`v12r08_ww`). Owns the
protobuf wire types, Mapperly mappers, and HTTP controllers for that
game version. Routes are under `/v12r08_ww/...`.
...
This is one of two parallel game-protocol adapters; `Adapters.GameProtocol.CnR00`
is the sibling for the CHN client.
...
- `Wire/` — protobuf-net `[ProtoContract]` types for the WwR08 wire
  format (request and response).
- `Mappers/` — `[Mapper]` partial classes generating the bidirectional
  WwR08 ↔ Common* conversion.
- `DependencyInjection.cs` — `AddGameProtocolWwR08()` extension that
  registers the controllers and any version-specific services.

## When to add code here

- A new endpoint that exists in the WwR08 client. Add the controller
  ...
  Common* DTO. Mirror in `Adapters.GameProtocol.CnR00` if the CHN
