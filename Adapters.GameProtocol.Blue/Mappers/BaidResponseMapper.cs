...
public static partial class BaidResponseMapper
{
    ...
    private static byte[] MapDanFlag(Ac15BaidResponse response)
        => BlueProtocolBytes.FixedOrZero(response.Dan?.GotDanFlg, BlueProtocolBytes.DanFlagBytes);

    private static byte[] MapDanExtraFlag(Ac15BaidResponse response)
        => BlueProtocolBytes.FixedOrZero(response.Dan?.GotDanExtraFlg, BlueProtocolBytes.DanExtraFlagBytes);
