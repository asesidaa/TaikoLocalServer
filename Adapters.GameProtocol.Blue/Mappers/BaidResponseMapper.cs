...
public static partial class BaidResponseMapper
{
    ...
    private static byte[] MapCostumeFlag(byte[]? value)
        => Ac15MapperNormalization.FixedOrZero(value, Ac15EraProfiles.Blue.Limits.CostumeFlagBytes);

    private static byte[] MapDanFlag(byte[]? value)
        => Ac15MapperNormalization.FixedOrZero(value, Ac15EraProfiles.Blue.Limits.DanFlagBytes);

    private static byte[] MapDanExtraFlag(byte[]? value)
        => Ac15MapperNormalization.FixedOrZero(value, Ac15EraProfiles.Blue.Limits.DanExtraFlagBytes);
}
