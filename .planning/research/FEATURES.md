...
| Implement Wai Wai from wiki only | Yellow wiki says update 81 added it. | Current Yellow proto scan did not expose matching `waiwai_*` fields. | Track as evidence gap and add only with route/proto/log proof. |
...
- [ ] Red support.
- [ ] Yellow Wai Wai behavior if local protocol/runtime evidence appears.
