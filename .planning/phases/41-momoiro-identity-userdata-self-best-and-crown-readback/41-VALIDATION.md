...
|---------|------|------|-------------|-----------------|-----------|-------------------|-------------|--------|
| 41-W0-01 | TBD | 0 | MORDB-01 | MOMOIRO baid/mydon flow creates shared identity plus `UserSaveData_Momoiro` only; adjacent era save tables remain untouched. | handler/controller integration | `dotnet test Tests/Tests.csproj --filter "FullyQualifiedName~MomoiroBaid|FullyQualifiedName~MomoiroMyDon" --no-restore -- RunConfiguration.DisableParallelization=true` | `Tests/Momoiro/MomoiroReadbackHandlerTests.cs` | pending |
| 41-W0-02 | TBD | 0 | MORDB-02 | `selfbest.php` reads only `SongBestDatum_Momoiro`, including normal, ura, and shin rows mapped through current MOMOIRO wire. | handler/controller integration | `dotnet test Tests/Tests.csproj --filter "FullyQualifiedName~MomoiroSelfBest" --no-restore -- RunConfiguration.DisableParallelization=true` | `Tests/Momoiro/MomoiroSelfBestTests.cs` | pending |
| 41-W0-03 | TBD | 0 | MORDB-03 | Favorite and recent readback uses MOMOIRO-owned rows, Phase 40 limits, stable ordering, truncation, and no adjacent-era leakage. | handler/controller integration | `dotnet test Tests/Tests.csproj --filter "FullyQualifiedName~MomoiroFavorite|FullyQualifiedName~MomoiroRecent|FullyQualifiedName~MomoiroUserData" --no-restore -- RunConfiguration.DisableParallelization=true` | `Tests/Momoiro/MomoiroUserDataTests.cs` | pending |
| 41-W0-04 | TBD | 0 | MORDB-04 | `userdata.php` returns `hash_crown_flg` packed by MOMOIRO catalog/hash order and does not expose `crownsdata.php`. | byte packing/regression | `dotnet test Tests/Tests.csproj --filter "FullyQualifiedName~MomoiroCrown|FullyQualifiedName~MomoiroRouteSurface" --no-restore -- RunConfiguration.DisableParallelization=true` | `Tests/Momoiro/MomoiroCrownReadbackTests.cs` | pending |
| 41-W0-05 | TBD | 0 | MORDB-05 | `userdata.php` returns MOMOIRO catalog-order release-song bytes and binary-backed song hash version/table. | handler/controller integration | `dotnet test Tests/Tests.csproj --filter "FullyQualifiedName~MomoiroRelease|FullyQualifiedName~MomoiroSongHash|FullyQualifiedName~MomoiroUserData" --no-restore -- RunConfiguration.DisableParallelization=true` | `Tests/Momoiro/MomoiroUserDataTests.cs` | pending |
...
- [ ] RED tests compile without future-symbol failures and fail only for missing MOMOIRO readback behavior.
- [ ] Tests cover shared identity plus MOMOIRO-owned save creation, and prove adjacent era saves remain untouched.
- [ ] Tests cover `userdata.php` and `selfbest.php` controller behavior through response objects, not source strings.
- [ ] Tests cover catalog/hash-order crown packing so high MOMOIRO song IDs are not dropped by raw song-id indexing.
- [ ] Tests cover absence of `crownsdata.php` and other unsupported/proto-only route families.
...
|----------|-------------|------------|-------------------|
| Exact native favorite ordering | MORDB-03 | Research did not prove whether the client has hidden ordering beyond stored list order for favorites; current implementation should use stable DB insertion/order pattern plus Phase 40 cap. | If new client/log evidence appears, update favorite ordering tests before changing production behavior. |
