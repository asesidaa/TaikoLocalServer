...
| 2 | BTL-02: Blue `battleuserdata.php` returns evidence-backed default and persisted battle state without unsafe zero-default fields. | VERIFIED | `Adapters.GameProtocol.Blue/Controllers/BattleUserDataController.cs` sends `GetBattleUserDataQuery`; `Application/Handlers/GetBattleUserDataQuery.Blue.cs` reads tokens and complete `BlueBattleNpcStates` rows, returning `MaxDpn` from the stored maximum; `Adapters.GameProtocol.Blue/Mappers/BattleUserDataMappers.cs` maps `LastSelectSpecial1/2/3` into `NpcDatas`; incomplete NPC rows stay omitted. |
| 3 | BTL-03: Blue `initialdatacheck.php` advertises battle availability and release flags according to the approved battle design. | VERIFIED | `Infrastructure/GameDataCatalog/Blue/BlueBattleDataLoader.cs` requires the five-file battle XML set, parses stage IDs, battle special move reward IDs from `battletokeninfo.xml` reward rows, and NPC cap; `Application/Handlers/GetInitialDataQuery.Blue.cs` derives stage/special bitsets and cap from `blue.BattleCatalog`, not hardcoded IDs, bitsets, or cap values. |
...
| `GetInitialDataQuery.Blue.cs` | `ReleaseBattleSpecialFlg` | `blue.BattleCatalog.ReleaseBattleSpecialIds` from parsed battle special move reward IDs in `battletokeninfo.xml` reward rows | Yes | VERIFIED |
...
| Host temp-output build | `dotnet build Host/Host.csproj -o "$env:TEMP\TaikoLocalServer-host-build-dpn-special-fix" --no-restore` | Succeeded, 0 warnings, 0 errors | PASS |
...
| BTL-02 | 05-07, gap closure commit `58ba47f9`, DPN fix | Blue `battleuserdata.php` returns evidence-backed default and persisted battle state without unsafe zero-default fields. | SATISFIED | Mediator-backed handler reads complete persisted NPC rows into `NpcDatas`, including selected specials and maximum observed DPN, and omits incomplete NPC rows. |
