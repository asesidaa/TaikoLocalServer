...
- [x] `Tests/Momoiro/MomoiroCatalogLoaderTests.cs` protects root-level catalog loading, required-file validation, song count 380, song hash version 538116869, and 760-byte encoded song-hash table behavior. Current status: RED until Momoiro catalog registration/loading is implemented.
- [x] `Tests/Momoiro/MomoiroProtocolLimitsTests.cs` protects explicit Momoiro profile limits, userdata-owned crown placement, unsupported feature absences, conservative favorite/recent caps, and inferred crown/default/release wire lengths as documented assumptions. Current status: RED until Momoiro profile lookup is implemented.
- [x] `Tests/Momoiro/MomoiroMetadataRouteTests.cs` protects catalog-backed `recommend.php`, `defaultsong.php`, `songhash.php`, `telopcheck.php`, `gettelop.php` behavior plus static `heartbeat.php` and `bookkeeping.php` success. Current status: RED until metadata handlers/controllers use Momoiro catalog behavior.
