...
- Load MOMOIRO customization catalogs from MOMOIRO-owned sidecars or first-run extraction when available; do not borrow adjacent-era customization data.
...
- MOMOIRO customization catalog endpoints should expose MOMOIRO-owned costume/title/tone metadata when sidecars or extraction input exist, and should not borrow KIMIDORI/Murasaki/Nijiiro catalogs. Profile settings edit the MOMOIRO-owned numeric save fields and unlock bitsets only for controls backed by MOMOIRO protocol fields.
