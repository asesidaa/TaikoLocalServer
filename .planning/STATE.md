...
- [Phase 18]: Plan 18-01 captured ChallengeCompe as a protocol candidate surface, later corrected away from Don Challenge semantics. — Red route/proto evidence proves protocol fields/routes only; Don Challenge behavior is server-side stage-derived progress.
...
- [Phase 20]: Red protocol challenge arrays are mapped as playresult facts, but no ChallengeCompe state/readback contract was implemented.
...
- [Phase ?]: [Phase 21 Plan 01] Public DonChare sources are product context only. — Local Red proto, route, mapper, data, runtime, and client evidence remain implementation authority; the Don Challenge implementation was later corrected to stage-derived progress.
- [Phase ?]: [Phase 21 Plan 01] Red Don Challenge starts as a disabled-safe catalog contract. — Empty challengecompe.php success remains separate protocol compatibility only; stateful plans must stay inside the evidence gate.
- [Phase ?]: [Phase 21 Plan 01] Unknown Don Challenge rule kinds are unsupported predicates. — The sidecar parser preserves unknown rules as non-executable instead of letting them advance progress.
- [Phase 21]: Red Don Challenge stateful progress is allowed only inside the evidence gate. — Current behavior writes Red-owned state from local sidecar tasks and non-Tokkun normal playresult stages, without opt-in or protocol marker requirements.
- [Phase 21]: Don Challenge task task_id and slot are the local sidecar schema and evaluator aliases for compe_id and track_no. — The catalog sidecar owns task identity locally; the shared evaluator consumes explicit aliases without inventing a public-source schema.
...
- [Phase 21]: Configured Don Challenge rewards grant immediately from active completed-task thresholds and mutate only Red release-song/title flags. — Reward behavior applies through Red-owned save flags only.
- [Phase 21]: Don Challenge reward-song locks are derived from active configured reward songs and cleared by the existing AC15 userdata locked-song path. — Locks stay in shared userdata behavior instead of Red wire/controller code.
- [Phase 21]: Red rewardcardcheck.php and rewardexecution.php remain compatibility-only and do not participate in Don Challenge reward grants. — Reward behavior is through playresult/userdata only.
- [Phase 21]: Red ChallengeCompe protocol readback is a stub: it does not opt users in, grant rewards, echo raw upload facts, or expose Don Challenge progress. — Dedicated Don Challenge AdminApi/WebUI surfaces own operator readback.
...
- [Phase 21]: Final corrected behavior has Red Don Challenge focused tests, protocol-stub tests, Red regression, full test suite, and temp-output Host build coverage. — Current Red Don Challenge support uses Red-owned sidecar/state, non-Tokkun normal playresult stages, configured reward grants/locks, and dedicated AdminApi/WebUI readback.
...
- [Phase 22]: Red AdminApi parity is limited to normal implemented surfaces. — Plan 22-01 intentionally excludes Red item-shop, battle, WaiWai, Banacoin, medal/shop-season, payment, coupon, wallet, and protocol ChallengeCompe editing surfaces.
...
- [Phase 22]: Don Challenge WebUI availability is capability-gated by AdminApi availability. — Navigation shows the Play Data link only when `api/{era}/DonChallenge/availability` reports an active Don Challenge bundle.
...
- [Milestone v1.4]: White 0.13 support starts with route/root/transport evidence because the live `.tools/white/EBOOT.ELF.i64` is nonzero (`129893515` bytes), and Plan 23-01 route-table evidence now proves the `/v07r00/chassis` game route prefix plus the approved no-state suffix allowlist; local `ST7100-1` data alone is not route proof.
...
- [Phase 25]: White unsupported mode sections remain absent. Mapperly-generated White playresult maps Tokkun, BlueBattle, and GreenGhost sections to null, and White userdata intentionally omits Tokkun tutorial and ChallengeCompe stateful readback.
